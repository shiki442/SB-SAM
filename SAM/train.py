...
def train_model(cfg):
    ...
    print(f"=========================== Starting Training ===========================")
    tqdm_epoch = tqdm(range(initial_step, cfg.training.n_iter), desc="Training: Optimizer=Adam")
    ...
    evaluate_fn = utils.get_evaluate_fn(cfg, dataset, state)
    # Evaluate the model
    err_V = evaluate_fn(x_pred)
