import torch
import time
from SAM import cfg, datasets, utils
    ...
    def Euler_Maruyama_sampler(score):
        ...
        with torch.no_grad():
            for time_step in tqdm(time_steps, desc="Sampling"):
                t = torch.ones(cfg.ntrajs_sample, device=cfg.device) * time_step
...
def get_sampling_fn(cfg, sde, sampling_eps):
    shape = (cfg.ntrajs_sample, cfg.nd)
    x_ref_all, _, ind_sam = datasets.get_data_params(cfg)
    x_ref_sam = x_ref_all[ind_sam]
    x_init = torch.flatten(x_ref_sam) + sde.prior_sampling(shape, cfg.device)
